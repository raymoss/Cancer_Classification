...
def draw_correlation(df, target_df):
	correlation_list = []
	...
	print(df.shape)
	# for i in features:
	# 	correlation_list.append(target_df['label'].corr(df[columns_list[i]], 'pearson'))
	# 	selected_column_list.append(columns_list[i])
	correlation_list = dict()
	for i in columns_list:
		correlation_list[i] = target_df['label'].corr(df[i], 'pearson')
	sorted_by_value = sorted(correlation_list.items(), key = lambda kv: kv[1], reverse = True)
	...
	x_pos = [i for i, _ in enumerate(x_axis[0:9])]

	plt.bar(x_pos, y_axis[0:9], color='green')
	plt.xlabel("Features")
	plt.ylabel("Correlation Coeffient")
	plt.title("Features vs Coeffient")

	plt.xticks(x_pos, x_axis[0:9])
...
class PredictionModel():
	...
	def run_model(self):
		...
		df.pop('file_id')
		target_df = pd.DataFrame(y_data, columns = ['label'])
		columns =df.columns
		#print (columns)
		X_data = df.values
